...
| `*Tier 3 Product Type` | Product reference override, else built-in taxonomy for Private Credit/CLN/Repack/Illiquid, else asset-class default. Unknown Structured FI falls back to Product/Structure text, then `UNCLASSIFIED_TIER3`. | Product reference/rules needed for other unknown or firm-specific tier 3 taxonomy. |
...
| `Site Code` | Legal reference lookup. | Needs legal CSV. |
...
| `*$ PC` | Asset-specific PC waterfall: PC reference, VA proxy, PB fee, commission, bank-minus-MSS, zero, or unresolved depending on settings. | Confirm PC policy and TRS FX convention. |
| `*$ VA/GNBV` | Asset-specific VA/GNBV source derivation. | Source columns required; otherwise unresolved. |
| `Sales Team (Coverage)` | Coverage reference `sales_team_coverage`, else source `SALETEAM` where parsed. | Illiquid/Repack and Structured Credit currently have no source Sales Team. |
...
| `Buy/Sell` | Structured FI source `New/Tap/Sell`; Collar/TRS `New/Unwind` maps to Buy/Sell; Illiquid/Repack and Structured Credit blank by default. | Status-to-Buy/Sell rule needed if required for Illiquid/Repack or Structured Credit. |
...
| `BTB Trade Site` | Legal reference lookup. | Needs legal CSV. |
...
| `Risk Book` | Legal reference lookup. | Needs legal CSV. |
...
|---|---|---|
| Structured FI - Rate | `First Trade Date`/`Trade Date`/`Date`, `Currency`/`CCY`, `Volume ('MM) USD` or accepted Linear Zero notional aliases or `Size (Org Curr)` x `FX rate`, `Total NNBV`/`GNBV in $`/`NNBV`, `ISIN Front`/`ISIN`/`SVCS No.`/`Summit ID`, `Maturity`, `FINAL CUSTOMER`/`Client`, `SALETEAM`, `Book`/`Booking`, `Trader`, `Issuer`, `Underlying`/`Ticker`, `Structure`/`Security`, `First Reoffer`/`Reoffer`, optional `New/Tap/Sell`. | Coverage/legal/PC reference-driven fields; Secondary CCY; 2025/2026 Buy/Sell if no `New/Tap/Sell` column. |
| Structured FI - Credit | Same Structured FI mapping as Rate. Current-layout `Product` text can route CLN to Structured Credit / Structured Credit / Credit Linked Note; CLN takes priority over Range Accrual wording when both appear. | Coverage/legal/PC reference-driven fields; Secondary CCY; 2025/2026 Buy/Sell if no `New/Tap/Sell` column. |
| Structured FI - FX | Same Structured FI mapping as Rate. Current-layout `Product = Range Accrual with Conversion` maps to Structured Rates / Interest Rate Linked Note -PPN / Range Accrual with Conversion; `Product = Linear Zero Callable Notes` keeps the original zero-linear rates taxonomy. | Coverage/legal/PC reference-driven fields; Secondary CCY; 2025/2026 Buy/Sell if no `New/Tap/Sell` column. |
| Structured FI - Unknown | Same Structured FI mapping, but classification is inferred from `Product`, `Underlying`, and `Structure`. Product-specific taxonomy now covers Linear Zero Callable Notes, Range Accrual with Conversion, and CLN. | Product reference/rules required for other unknown taxonomy; coverage/legal/PC reference-driven fields. |
| Illiquid Credit | `Trade Date`, `Ccy`/`Currency`, `Volume ('MM) USD` or `Size (Org Curr)` x `FX Rate`, `GNBV (USD)`/`NNBV`, `ISIN`/`SVCS No.`, `Maturity`, `FINAL CUSTOMER`, `Booking`, `Trader`, `Issuer`, `BBG Tix 1`, `Deal Name`, `Reoffer`; tiers default to Structured Credit / Structured Credit / Structured Credit Notes. | Sales Team/Coverage if not in reference; legal/PC reference fields; Buy/Sell is blank unless `Status` is deliberately mapped. |
| Repack | Same Illiquid/Repack mapping, classified by Product Type or Deal Name containing Repack; tiers default to Structured Credit / Structured Credit / Structured Credit Notes. | Sales Team/Coverage if not in reference; legal/PC reference fields; Buy/Sell is blank unless `Status` is deliberately mapped. |
| Structured Credit | `Product`, `Category`, `Region`, `Notional (USD)`/`Notional`, `GNBV (USD)`/`GNBV`; currency policy defaults to `USD`; trade ID is deterministic numeric. CLN text maps to Structured Credit / Structured Credit / Credit Linked Note. | Trade Date, native Trade ID, Sales Client, Sales Team, Book, Trader, Legal Entity, Site Code, ISIN, Maturity, Issuer, Ticker, Buy/Sell, Price, and most reference-driven fields. |
...
| Equity TRS | `Trade Date`, `Currency`, `Notional in USD`, `MSS Revenue in USD` or `Total Bank Revenue in USD`, `Reference number`, `Underlying`, `Net Price`/`Gross Price`, `Structure`, `Maturity`, `Book`, `SALETEAM`, `FINAL CUSTOMER`, `New/Unwind`/`Status`; PC from `Commission to PB (HKD)` x FX rate or Bank-MSS. | Confirm PC FX-rate direction; legal/reference fields; ISIN; Issuer; Trader; Secondary CCY. |
