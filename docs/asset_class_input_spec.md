...
| `Product Type` (else `Underlying`+`Structure` regex fallback) | Optional -- classification only, never blocks a row (default falls to `Structured FI - Unknown` + tier3 = `Product`/`Structure` text or `UNCLASSIFIED_TIER3` placeholder) | `*Tier 1/2/3 Product Type` (via `classify()`/`BUILT_IN_PRODUCT_TAXONOMY`) |
